...
class ZenTao:
    ...
    def create_build(self, execution: int, product: int, project: int, name: str, builder: str, source: str, download: str,
                     files: list, desc: str):
        """
        获取版本
        :return:
        ...
        respond = self.session.get(
            self.host + '/build-create-{0}-{1}-{2}.json?{3}={4}'
            .format(str(execution), str(product), str(project), self.session_name, self.session_id),
            params=data)
        print(respond.url)
        if respond.status_code != 200:
            warnings.warn('http error: %d' % respond.status_code)
...
if __name__ == '__main__':
    z = ZenTao('https://pms.inrobot.cloud')
    if z.login('zhouqi', 'Infore123.'):
        print('登录成功')
    # if z.get_build(594):
    #     print('获取版本成功')
    if z.create_build(325, 74, 322, 'test', 'admin', 'github.com', 'ftp://192.168.1.1',
                      '', 'describe'):
        print("创建版本")
    if z.logout():
        print('注销成功')